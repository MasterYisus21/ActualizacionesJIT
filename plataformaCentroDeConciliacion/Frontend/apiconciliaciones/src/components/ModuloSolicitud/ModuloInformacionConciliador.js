...
function ModuloInformacionConciliador() {
  ...
  const agregarConciliador = (value) => {
    // event.preventDefault()
    axiosApiInstance.post(config.apiGatewayURL + "/solicitudes/" + UrlParams["Id_solicitud"] + "/conciliadores/" + value)
      .then((response) => {
        setIsOpen(false)
  ...
  return (
    <>
      <div className='container container-conciliador pt-3'>
        ...
        <div className='contenedor-navbar-agregar-conciliador'>
          <nav className="navbar navbar-light ">
            <div className="container-fluid">
              <form className="d-flex input-group w-auto align-items-sm-baseline gap-1" onSubmit={e => {e.preventDefault(); agregarConciliador(e.target.cedula.value)}}>
                <input
                  type="text"
        ...
        {isOpen &&
          <form className='contenedor-tabla-seleccion-conciliador mb-5 p-4 pb-3' onSubmit={e => {e.preventDefault(); agregarConciliador(e.target.identificacionPersona.value)}}>
