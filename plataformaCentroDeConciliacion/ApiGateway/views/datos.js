...
datosPersonas.ExportarDatosPersona = async (req, response) => {
    let datos = {}
    try {
        const personas = await axios.get(config.urlGateway + "solicitudes/" + req.params.id + "/" + response)
        if (personas.data == '' | personas.data == null) { return {} }
...
datosPersonas.ExportarDatos = async (req, response) => {
    try {
        ...
        if(Object.keys(search.data).length==0){return datos = {}}
        ...
        if (response == "hechos") {
            ...
            if (search.data.Tipo_resultado_Id != '') { search.data.Tipo_resultado_Id = search.data.Tipo_resultado_Id.Nombre;  }
...
datosPersonas.SolicitudesEspecificas = async (response) => {
    ...
    try {
        ...
        if (response.data != 0) {
            ...
            if (informacion_data.Tipo_resultado_Id != ''){informacion_data.Tipo_resultado_Id = tipos_resultado.data}
...
datosPersonas.EliminarPersonasDeCitacion = async (req) => {
    ...
    try {
        ...
        for await (const informacion_data of req.body) {
            ...
            const resp = await axios.get(config.urlApiConciliacion + "/relaciones_citacion_persona?Citacion_Id=" + req.params.id2 + "&Persona_Id=" + personas.data[personas.data.length - 1].Id);
